...
// Ya no hace falta que sea global, pero si quieres dejarlo global no pasa nada.
// Lo dejamos como en tu estilo, pero lo vamos a INICIALIZAR y a METERLO EN LA TUPLA.
...
int localiser::run(int argc, char* argv[])
{
    ...
    // mnist_proxy es global, no hace falta redeclarar aquí

    //Require code
    require<RoboCompCamera360RGB::Camera360RGBPrx, RoboCompCamera360RGB::Camera360RGBPrxPtr>(
        communicator(),
        configLoader.get<std::string>("Proxies.Camera360RGB"),
        "Camera360RGBProxy",
        camera360rgb_proxy
    );
    require<RoboCompLidar3D::Lidar3DPrx, RoboCompLidar3D::Lidar3DPrxPtr>(
        communicator(),
        configLoader.get<std::string>("Proxies.Lidar3D"),
        "Lidar3DProxy",
        lidar3d_proxy
    );
    require<RoboCompOmniRobot::OmniRobotPrx, RoboCompOmniRobot::OmniRobotPrxPtr>(
        communicator(),
        configLoader.get<std::string>("Proxies.OmniRobot"),
        "OmniRobotProxy",
        omnirobot_proxy
    );

    // --------- AQUI ESTABA EL FALLO: faltaba MNIST ----------
    require<ROBOCOMPMNIST::MNISTPrx, ROBOCOMPMNIST::MNISTPrxPtr>(
        communicator(),
        configLoader.get<std::string>("Proxies.MNIST"),
        "MNISTProxy",
        mnist_proxy
    );

    // OJO: esto requiere que TuplePrx tenga 4 proxies (incluyendo MNIST)
    tprx = std::make_tuple(camera360rgb_proxy, lidar3d_proxy, omnirobot_proxy, mnist_proxy);
