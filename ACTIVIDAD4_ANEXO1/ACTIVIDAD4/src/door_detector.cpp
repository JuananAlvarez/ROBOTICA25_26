...
Doors DoorDetector::detect(const RoboCompLidar3D::TPoints &points, QGraphicsScene *scene)
{
    ...
    // ✅ ORDENAR para que doors_cache[0] sea SIEMPRE la puerta más cercana
    std::sort(detected_doors.begin(), detected_doors.end(),
...
/*
 *-----------------------------------------------------------------------------
 *NUEVO FILTER POINT DANIEL JIMENEZ 15/12
 *ACTIVIDAD 4
 *Con esto, el conjunto resultante no puede tener puntos duplicados y respta exactamente
 *  la semantica que ya tenias, solo que con la inversión de bucles vale,
 *-----------------------------------------------------------------------------
 */
RoboCompLidar3D::TPoints DoorDetector::filter_points(const RoboCompLidar3D::TPoints &points,
                                                     QGraphicsScene *scene)
{
    // Detectar puertas y cachearlas
    const auto doors = detect(points, scene);
    if (doors.empty())
        return points;
    ...
    // Recorremos primero los puntos, luego todas las puertas
    for (const auto &p : points)
    {
        ...
        for (const auto &d : doors)
        {
            ...
            // ¿El rango angular de la puerta envuelve -pi/+pi?
            const bool angle_wraps = d.p2_angle < d.p1_angle;

            bool point_in_angular_range = false;
            if (angle_wraps)
            {
                // Caso envolvente: está en rango si phi > p1 || phi < p2
                point_in_angular_range = (p.phi > d.p1_angle) || (p.phi < d.p2_angle);
            }
            else
            {
                // Caso normal: phi entre p1 y p2
                point_in_angular_range = (p.phi > d.p1_angle) && (p.phi < d.p2_angle);
            }

            // Si el punto está en el “hueco de la puerta” y más lejos que la puerta, se descarta
            if (point_in_angular_range && p.distance2d >= dist_to_door)
            {
                remove_point = true;
                break;  // no hace falta seguir comprobando otras puertas
            }
        }
